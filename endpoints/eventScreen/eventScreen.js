...
const getEvent = async (request, response) => {
    ...
    try {
        ...
        await logger(request, response, "Info", "Received information for event with id: " + id);
        response.status(200).json({ 'event': event, 'performers': performers, 'comments': comments });
    } catch (error) {
        await logger(request, response, "Error", "Error receiving event (" + id + ") information: " + error.message);
...
const getUpdatedComments = async(request, response) => {
    ...
    try {
        const comments = await getEventComments(request, response, id)
        await logger(request, response, "Info", "Received updated comments for event with id: " + id);
